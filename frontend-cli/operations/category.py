...
class Category:
    ...
    def category_create(self, user_id, budget_id, group_id=None) -> None:
        """
        Prompt for and create a new category
        """
        name = input("Category Name: ")
        time_allocated = get_interval("Time Allocated").total_seconds()
        category = {"category_name": name, "time_allocated": time_allocated, "group_id": group_id}
    ...
    def category_update(self, user_id, budget_id, category_id: int) -> None:
        ...
        time_allocated = get_interval("Updated time allocated: ").total_seconds()
    ...
    def category_list(self, user_id, budget_id, group_id=None) -> list[dict[str, Any]] | None:
        ...
        """
        if group_id is None:
            response = self.api_handler.get_api(f"users/{user_id}/budgets/{budget_id}/categories?detailed=true")
        else:
            response = self.api_handler.get_api(f"users/{user_id}/budgets/{budget_id}/groups/{group_id}/categories?detailed=true")
        return response
