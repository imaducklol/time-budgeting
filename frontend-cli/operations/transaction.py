...
class Transaction:
    ...
    def transaction_create(self, user_id, budget_id, group_id=None):
        """
        Prompt for and create a transaction
        """
        transaction_name = input("Transaction name: ")
        period = get_interval("Period: ")

        transaction = {"transaction_name": transaction_name, "period": period, "group_id": group_id}
        response = self.api_handler.post_api(f"users/{user_id}/budgets/{budget_id}/transactions", transaction)

        # TODO Do we need to unset up_to_date?

    def transaction_update(self, user_id, budget_id, transaction_id, group_id=None) -> None:
        """
        ...
        period = get_interval("Updated period: ")
        transaction = {"transaction_name": transaction_name, "period": period, "group_id": group_id}
        response = self.api_handler.patch_api(f"users/{user_id}/budgets/{budget_id}/transactions/{transaction_id}",
                                              transaction)

    def transaction_delete(self, user_id, budget_id, transaction_id) -> None:
        """
        ...
        if validate_choice("Are you sure you want to delete this transaction?"):
            response = self.api_handler.delete_api(f"users/{user_id}/budgets/{budget_id}/transactions/{transaction_id}")
